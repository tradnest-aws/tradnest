...
- **2026-09-05.** Vendor `/seller/login` and `/seller/register` switcher copy was clipped (`mt-auto` + flex `min-h-0`) and low-contrast (`text-ui-fg-muted` / navy interactive on dark). Footers sit under the form; `.auth-switch-copy` is navy on light and canvas on dark, gold underlined links. Login always shows “create account”. Redeploy vendor: `./scripts/deploy-ec2-tradnest.sh nginx`.
